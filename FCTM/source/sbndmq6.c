...
#define GRAM6(j) (B[gety(y,j)]<<5)&(B[gety(y,j-1)]<<4)&(B[gety(y,j-2)]<<3)&(B[gety(y,j-3)]<<2)&(B[gety(y,j-4)]<<1)&B[gety(y,j-5)]
...
int search_large(unsigned char *x, int m, dtype *y, int n) {
   ...
   while (j+diff < n) {
      ...
      if (D != 0) {
         ...
         j += mq;
         if (j == pos) {
            for(i=m+1; i<p_len && x[i]==gety(y,j-m+1+i); i++);
