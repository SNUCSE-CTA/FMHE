...
const int q=4;
#define simd_comp_block(y,i) (simd_comp(y,i)+(simd_comp(y,(i)+4)<<4))
#define si_block(i) (si[i]+(si[(i)+4]<<4))
#else
inline int simd_comp(dtype *y, int i){
	__m128i am = _mm_loadu_si128((__m128i *)(y+i));
	__m128i bm = _mm_loadu_si128((__m128i *)(y+i+1));
	__m128i rm = _mm_cmpgt_epi8(am, bm);
	return _mm_movemask_epi8(rm) & 0xff;
}
const int q=bsize;
...
//  }
  /* Searching */
   int i=mMb;
   const int nMb1=n-bsize+1;
   int j;
   while (true) { 
//	for(int i=mMb;i<nMb1;i++){
	   do {
