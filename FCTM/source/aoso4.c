...
int search_large(unsigned char *x, int m, dtype *y, int n, int q);

void verify(dtype *y, int j, int n, unsigned char *x, int m, int q, unsigned int D, unsigned int mm, int &count) {
    unsigned int s;
    int c, k, i;
    D = (D & mm)^mm;
    while (D != 0) {
        ...
        c = -(m/q-1)*q-s/(m/q);
        k = 0; i=j+c;
        if(i>=0 && i<=n-m) while(k<m && x[k]==gety(y, i+k)) k++;
        if(k==m) OUTPUT(i);
        D &= ~(1<<s);
    }
}
...
void verify_large(dtype *y, int j, int n, unsigned char *x, int m, int q, unsigned int D, unsigned int mm, int &count, int p_len) {
    unsigned int s, tmp;
    int c, k, i;
    D = (D & mm)^mm;
    while (D != 0) {
        ...
        c = -(m/q-1)*q-s/(m/q);
        k = 0; i=j+c;
        if(i>=0 && i<=n-p_len) while(k<p_len && x[k]==gety(y, i+k)) k++;
        if(k==p_len) OUTPUT(i);
        D &= ~(1<<s);
    }
}
